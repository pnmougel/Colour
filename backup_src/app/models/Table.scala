...
abstract class Table[T] {
    def all(orderBy : Option[String] = None, 
            order : Option[String] = None, 
            limit : Option[Int] = None,
            offset : Option[Int] = None): List[T] = DB.withConnection { implicit c =>
    ...
    def findWhere(params: Seq[(String, ParameterValue[Any])]): Seq[T] =
        DB.withConnection { implicit connection =>
            val query = "SELECT * FROM " + tableName + (if (params.isEmpty) {
                ""
            } else {
                params.map { case (col, _) => col + " = {" + col + "}" }.mkString(" AND ")
            }
        )
        SQL(query).on(params:_*).as(single *)
    }
