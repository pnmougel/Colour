...
object UserVote {
    ...
    def hasVotedForConference(conferenceId: Long, userId: Long) : Boolean = DB.withConnection { implicit c =>
        val row = SQL("SELECT * FROM scoreuser WHERE conference_id = {id} AND user_id = {userId}").on(
                'id -> conferenceId,
    ...
    def getByConferenceId(conferenceId: Long) : List[UserVote] = DB.withConnection { implicit c =>
        SQL("SELECT * FROM scoreuser WHERE conference_id = {id}").on('id -> conferenceId).as(userVote *)
    }
    ...
    def create(conferenceId : Long, userId: Long, score: Int, date : Date) {
        DB.withConnection { implicit c =>
            SQL("INSERT INTO scoreuser (conference_id, user_id, score, date) values " +
    ...
    def delete(conferenceId: Long, userId: Long) = DB.withConnection { implicit c =>
    	SQL("DELETE FROM scoreuser WHERE conference_id = {conferenceId} AND user_id = {userId}").on(
                    'conferenceId -> conferenceId,
    ...
    def deleteByConference(conferenceId : Long) = DB.withConnection { implicit c =>
        SQL("DELETE FROM scoreuser WHERE conference_id = {conferenceId}").on(
