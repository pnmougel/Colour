...
object Admin extends Controller {
    ...
    def run(actionName: String) = Action { implicit request =>
        if(actionsMap.contains(actionName)) {
            val promiseOfResult = Akka.future { actionsMap(actionName).run(request) }
            Async {
                promiseOfResult.map {i => Ok("")}
            }
        } else {
