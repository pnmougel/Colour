...
object Ranking extends Controller {
    ...
    def byField(fieldId : Long) = Action { implicit request =>
        ...
        Ok(views.html.ranking(fieldId, conferences, subFields, 1, nbConferencesPerPage))
    }
    ...
    def getCTypes(isNational : Boolean, isInternational : Boolean, 
        ...
        if(isConference) {
            if(isInternational) {
                cTypes.push(models.CType.getOrCreate("Conference"))
            }
            if(isNational) {
            	cTypes.push(models.CType.getOrCreate("Conference"))
            }
        ...
        if(isJournal) {
            if(isInternational) {
                cTypes.push(models.CType.getOrCreate("Journal"))
            }
            if(isNational) {
            	cTypes.push(models.CType.getOrCreate("Journal"))
            }
        ...
        if(isWorkshop) {
            if(isInternational) {
                cTypes.push(models.CType.getOrCreate("Workshop"))
            }
            if(isNational) {
            	cTypes.push(models.CType.getOrCreate("Workshop"))
            }
