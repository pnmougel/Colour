...
object ExternalScore extends Table[ExternalScore](Option("conference_externalranking")) {
    ...
    def createOrUpdate(
            ...
            score: Double, 
            scoreText : String) = DB.withConnection { implicit c =>
        ...
        if(externalScore.isDefined) {
            if(score > externalScore.get.score) {
                updateWhere(Where('conference_id -> conferenceId, 'externalranking_id -> externalrankingId), 'score -> score, 'score_text -> scoreText)
            }
        } else {
            createEntry(
