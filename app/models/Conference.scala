...
case class Conference(name: String, shortName: String,
    ...
    lazy val relatedConferences = DB.withConnection { implicit c => SQL("""
    ...
    lazy val comments = Comment.select(Where('conference_id -> id) :: OrderBy("created_at", "DESC"))
    ...
    def addSubField(subfieldId: Long) = DB.withConnection { implicit c =>
    ...
    def removeSubField(subfieldId: Long) = DB.withConnection { implicit c =>
        SQL("DELETE FROM conference_subfield WHERE conference_id = {conferenceId} AND subfield_id = {subfieldId}").on(
            'conferenceId -> id, 'subfieldId -> subfieldId).executeUpdate()
    }
    lazy val updates = Update.select(Where('conference_id -> id) :: OrderBy("created_at", "DESC") :: Limit(Update.defaultHistorySize))
    lazy val updatesAll = Update.select(Where('conference_id -> id) :: OrderBy("created_at", "DESC"))
}
...
object Conference extends Table[Conference] {
    ...
    def getPage(fieldId: Long, ctypes: List[Long], subFields: List[Long], nbConferencesPerPage: Option[Int], startAt: Option[Int], orderBy: String, sort: String): List[Conference] = DB.withConnection { implicit c =>
        val subFieldsStr: String = if (subFields.size > 0) subFields.mkString("(", ",", ")") else " (NULL) "
        val ctypesStr: String = if (ctypes.size > 0) ctypes.mkString("(", ",", ")") else " (NULL) "
    ...
    def countPublications(query: String, shortName : Option[String] = None, ctypeId : Option[Long] = None, fieldId : Option[Long] = None) = DB.withConnection { implicit c =>
    ...
    private def buildWhereQuery(test : String, shortName : Option[String] = None, ctypeId : Option[Long] = None, fieldId : Option[Long] = None) : String = {
        ...
        if (ctypeId.isDefined) { filter += " AND ctype_id = {ctypeId} "}
        if (fieldId.isDefined) { filter += " AND field_id = {fieldId} "}
        if (shortName.isDefined) { filter += " AND short_name_lower = {shortName} "}
        filter
    }
    def findPublications(query: String, shortName : Option[String] = None, ctypeId : Option[Long] = None, 
            fieldId : Option[Long] = None, allResults : Boolean = false) : List[(Conference, Double)] = DB.withConnection { implicit c =>
        val queryLower = query.toLowerCase()
        ...
        if(publications.isEmpty || allResults) {
            ...
            Utils.cleanName(query + " " + shortName).split(" ").foreach { stems.add(_) }
            publications = publications ::: Stem.getMatchingConferences(stems).map { case (conferenceId, matchingScore) => 
     ...
     */
    def getQuickMatch(query: String): List[Conference] = DB.withConnection { implicit c =>
        SQL("""
    ...
    def create(name: String, shortName: String, ctypeId: Long, fieldId: Long, regionIdOpt: Option[Long] = None, userIdOpt: Option[Long] = None): Long = {
        val regionId = if (regionIdOpt.isDefined) regionIdOpt.get else Region.getOrCreate("Worldwide", None, true)
    ...
    def update(id: Long, name: String, shortName: String, ctypeId: Long, regionId: Long, yearSince: Option[Int], publisherId: Option[Long],
               description: Option[String], subfieldsToAdd: List[Long], subfieldsToRemove: List[Long], userId: Long) = DB.withConnection { implicit c =>
        ...
        if (name != conference.name || shortName != conference.shortName || description != conference.description || ctypeId != conference.ctypeId ||
            ...
            subfieldsToAdd.foreach { subfieldId =>
                val isAlreadyPresent = SQL("SELECT count(*) FROM conference_subfield WHERE conference_id = {conferenceId} and subfield_id = {subfieldId}").on(
                    'conferenceId -> id, 'subfieldId -> subfieldId).as(scalar[Long].single)
                if (isAlreadyPresent == 0) {
                    SQL("INSERT INTO conference_subfield (conference_id, subfield_id) VALUES ({conferenceId}, {subfieldId})").on(
            ...
            subfieldsToRemove.foreach { subfieldId =>
                SQL("DELETE FROM conference_subfield WHERE conference_id = {conferenceId} AND subfield_id = {subfieldId}").on(
                    'conferenceId -> id, 'subfieldId -> subfieldId).executeUpdate()
    ...
    def getConferenceRankFromScore(scores: HashMap[Conference, Int]): HashMap[Conference, (Int, Double)] = {
        // Group the conference by subfields
        var subFieldToConferences = new HashMap[SubField, Stack[(Conference, Int)]]()
        scores.foreach { case (conference, scoreValue) =>
            val subfields = conference.subfields
            subfields.foreach { subfield =>
                subFieldToConferences.getOrElseUpdate(subfield, new Stack[(Conference, Int)]()).push((conference, scoreValue))
            }
        }

        // Compute the score
        var conferenceRanks = new HashMap[Conference, (Int, Double)]()
