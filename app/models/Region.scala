...
object Region extends Table[Region] {
    def getInternationalRegions() = select(Where('is_international -> true) :: OrderBy("id"))
    
    def getCountries() = select(Where('is_international -> false) :: OrderBy("name"))
    
    lazy val getFrequentCountries : List[Region] = DB.withConnection { implicit c =>
        /*
        SQL("""SELECT r.*
