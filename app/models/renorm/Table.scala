...
abstract class Table[T](tableNameOpt : Option[String] = None)(implicit t: reflect.Manifest[T]) {
    ...
    val constructorFieldNames : Array[String] = fields.slice(0, nbFields) map { field =>
    ...
    private def defaultRowParser : RowParser[T] = RowParser.apply[T]( { r : Row =>
        val rowMap = r.asMap
        var constructorArgs : Array[java.lang.Object] = constructorFieldNames.map { fieldName =>
        ...
        try {
            ...
            case e : IllegalArgumentException => {
                // println("Unable to map " + constructorFieldNames.mkString("(", ",", ")") + " to " + constructorArgs.mkString("(", ",", ")"))
    ...
    def createEntry(params : (Any, ParameterValue[_])*) : Long = DB.withConnection { implicit c =>
     ...
     * FIND
     */
    def findIdWhere(params : (Any, ParameterValue[_])*): Option[Long] = DB.withConnection { implicit connection =>
        val query = "SELECT " + idColumn + " FROM " + tableName + " WHERE " + andQuery(params:_*)
        SQL(query).on(params:_*).as(scalar[Long].singleOpt)
    }
    
    def find(params : (Any, ParameterValue[_])*) = DB.withConnection { implicit connection => 
        query(Where(params:_*)).as(single *)
    }
    def findOption(params : (Any, ParameterValue[_])*) = DB.withConnection { implicit connection => 
        query(Where(params:_*)).as(single.singleOpt)
    }
    def findOne(params : (Any, ParameterValue[_])*) = DB.withConnection { implicit connection =>
        query(Where(params:_*)).as(single.single)
    }

    def byId(id : Long) = DB.withConnection { implicit connection =>
        query(Where('id -> id)).as(single.singleOpt)
    }
    
    /**
     * SELECT QUERY
     */
    def select(q : Query = EmptyQuery) = DB.withConnection { implicit connection => query(q).as(single *) }
    def selectOne(q : Query = EmptyQuery) = DB.withConnection { implicit connection => query(q).as(single.single) }
    def selectOption(q : Query = EmptyQuery) = DB.withConnection { implicit connection => query(q).as(single.singleOpt) }
    ...
    def deleteWhere(params : (Any, ParameterValue[_])*) : Long = DB.withConnection { implicit c =>
     ...
     * UPDATES
     */
    def update(id : Long, params : (Any, ParameterValue[_])*) = DB.withConnection { implicit c =>
        SQL("UPDATE " + tableName + " SET " + mergeParams(", ", params:_*) + " WHERE " + idColumn + " = " + id).on(params:_*).executeUpdate()
    }
    
    def updateWhere(where : Where, params : (Any, ParameterValue[_])*) = DB.withConnection { implicit c =>
     ...
     * COUNT
     */
    def countWhere(params : (Any, ParameterValue[_])*) : Long = DB.withConnection { implicit c =>
     ...
     */
    def oneToMany[FT](fTable : Table[FT], foreignKey : ForeignKey = ForeignKeyGenerate)(implicit instance : T) : List[FT] = DB.withConnection { implicit c =>
    ...
    def manyToOneOpt[FT](fTable : Table[FT], foreignKey : ForeignKey = ForeignKeyGenerate)(implicit instance : T) : Option[FT] = DB.withConnection { implicit c =>
        val foreignKeyField = foreignKey match {
     ...
     * PRIVATE
     */
    private def query(query : Query = EmptyQuery) : SimpleSql[_] = DB.withConnection { implicit connection =>
