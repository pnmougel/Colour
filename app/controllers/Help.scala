...
object Help extends Controller {
    ...
    def linktoName(link : String) = {
        if(link == "api") { "API" } else { WordUtils.capitalizeFully(link.replace("_", " "), '#') }
    }
            
    def section(name : String) = Action { implicit request =>
        val lowerCaseName = nametoLink(name)
        if(menuSections.exists { case (section, subsections) => {
