...
object Scores extends Controller {
    ...
    def addVote = Authentification { implicit request =>
        userScoreForm.bindFromRequest.fold(
        	errors => BadRequest("How did you manage that ?"),
        	params => params match { case (conferenceId, score) =>
    ...
    def deleteVote = Authentification { implicit request =>
        deleteVoteForm.bindFromRequest.fold(
        	errors => BadRequest("How did you manage that ?"),
        	params => params match { case (conferenceId) =>
        	    if(request.user.isDefined) {
